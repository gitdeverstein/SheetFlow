...
import { DonutChart, StatusPill, OverflowMenu } from './DashboardComponents.js';
  ...
  return (
    <div className="space-y-8">
                    ...
                    <tr key={quote.id} className="hover:bg-slate-900/30 transition-colors">
                      ...
                      <td className="py-3.5">
                        ...
                        <OverflowMenu>
                          ...
                          <button onClick={() => setDeleteConfirmId(quote.id)} className="w-full text-left px-3 py-2 text-xs text-rose-400 hover:bg-slate-800 flex items-center gap-2"><Trash2 size={13} /> Delete</button>
                        </OverflowMenu>
                      </td>
                    </tr>
                  ))}
      ...
      <ConfirmModal
        ...
        onConfirm={() => { if (deleteConfirmId) deleteQuote(deleteConfirmId); }}
        title="Delete Quote"
        message="Are you sure you want to delete this quote? If it was accepted, inventory stock will be restored."
        confirmLabel="Delete"
        variant="danger"
      />
      ...
      <ConfirmModal
        ...
        onConfirm={() => { if (statusConfirm) updateQuoteStatus(statusConfirm.id, statusConfirm.status); }}
        title="Change Quote Status"
        message={`Are you sure you want to change the status to "${statusConfirm?.status}"? This will adjust your inventory stock levels.`}
