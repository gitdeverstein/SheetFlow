...
export default function QuoteGenerator() {
  ...
  // Add empty item line
  const handleAddItem = () => {
    setItems((prev) => [...prev, { productId: '', quantity: 1, unitPrice: 0, name: '' }]);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-2">
            {editingQuoteId
              ? 'Modify an existing quote and save changes.'
              : 'Compile products, assign clients, and calculate total values.'}
          </p>
        </div>
        {editingQuoteId && (
          <button
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-2 glass-panel p-6 rounded-2xl space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-300">
                Valid Until <span className="text-slate-500 font-normal">(optional)</span>
              </label>
              <input
                type="date"
                value={validUntil}
