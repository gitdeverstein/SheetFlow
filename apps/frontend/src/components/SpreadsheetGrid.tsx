...
export default function SpreadsheetGrid({ tab }: SpreadsheetGridProps) {
    ...
    () =>
      rows.filter((row) =>
        columns.every((col) => {
          ...
          return String(row.cells[col.id]?.value || '')
            .toLowerCase()
            .includes(filterVal);
        }),
      ),
  ...
  const handleCsvImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (lines.length < 2) {
      useSheetStore.getState().addToast('CSV must have a header and at least one row', 'error');
      return;
    }
    const headers = lines[0].split(',').map((h) => h.trim().toLowerCase().replace(/\s+/g, ''));
    const missing = ['sku', 'name', 'stock', 'alertthreshold', 'price'].filter((r) => !headers.includes(r));
    if (missing.length > 0) {
      useSheetStore.getState().addToast(`CSV missing columns: ${missing.join(', ')}`, 'error');
      return;
    }
    const parsed = lines
      .slice(1)
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-2">
            Double-click cells to edit. Starting with <code className="text-xs text-brand-400">=</code> evaluates
            formulas.
          </p>
        </div>
          ...
          <div className="min-w-[800px]">
            ...
            >
              {columns.map((col) => (
                <div
                  key={col.id}
                  className="p-3 text-sm font-semibold text-slate-300 flex items-center justify-between border-r border-slate-800/50"
                >
            ...
            {/* Filter Row */}
            <div
              className="grid border-b border-slate-800 bg-slate-950/20"
              style={{ gridTemplateColumns: `repeat(${columns.length}, 1fr) 100px` }}
            >
              {columns.map((col) => (
              ...
              {loading || isCustomersLoading || isInventoryLoading ? (
                ...
                <div className="p-12 flex flex-col items-center gap-3 text-center">
                  ...
                  {rows.length === 0 && (
                    <button
                      onClick={() => addNewRow(tab)}
                      className="mt-2 inline-flex items-center gap-2 px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium rounded-xl transition-all"
                    >
                      ...
                      return (
                        <div
                          ...
                          style={
                            {
                              ...style,
                              gridTemplateColumns: `repeat(${columns.length}, 1fr) 100px`,
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'ArrowDown') {
                              e.preventDefault();
                              const next = paginatedRows[index + 1];
                              if (next)
                                handleCellClick(next.id, columns[0]?.id, String(next.cells[columns[0]?.id]?.raw || ''));
                            } else if (e.key === 'ArrowUp') {
                              e.preventDefault();
                              const prev = paginatedRows[index - 1];
                              if (prev)
                                handleCellClick(prev.id, columns[0]?.id, String(prev.cells[columns[0]?.id]?.raw || ''));
                            } else if (e.key === 'Tab') {
                              e.preventDefault();
                              ...
                              if (e.shiftKey) {
                                if (colIdx > 0)
                                  handleCellClick(
                                    row.id,
                                    columns[colIdx - 1].id,
                                    String(row.cells[columns[colIdx - 1].id]?.raw || ''),
                                  );
                              ...
                              } else if (index < paginatedRows.length - 1) {
                                const nr = paginatedRows[index + 1];
                                handleCellClick(nr.id, columns[0].id, String(nr.cells[columns[0].id]?.raw || ''));
                              }
                            }
                          }}
                        >
                              ...
                              >
                                {col.type === 'select' && col.options ? (
                                  <select
                                    ...
                                    onChange={(e) => {
                                      updateSpreadsheetCell(tab, row.id, col.id, e.target.value);
                                      saveSpreadsheetRow(tab, row.id);
                                    }}
                                    onClick={(e) => e.stopPropagation()}
                                  ...
                                  <div className="w-full flex items-center justify-between">
                                    ...
                                    {hasFormula && (
                                      <span className="text-[9px] px-1 bg-cyan-500/10 text-cyan-400/80 rounded border border-cyan-500/20 opacity-0 group-hover:opacity-100 transition-opacity font-mono">
                                        fx
                                      </span>
                                    )}
                                  </div>
